...
async function renderA4VoucherHTML(data, agencySettings) {
  ...
  // Agency logo HTML with explicit max-width constraint to prevent breaking layout
  let logoHtml = '';
  if (agencySettings.logo && agencySettings.logo.startsWith('data:image')) {
    logoHtml = `<img src="${agencySettings.logo}" style="max-height: 35px; max-width: 110px; object-fit: contain;" alt="Logo">`;
  } else {
    logoHtml = `
  ...
  // 1. Passenger Basic Details Table Rows (Ultra-Compact)
  ...
  return `
    <div style="width: 100%; max-width: 100%; box-sizing: border-box; page-break-inside: avoid; break-inside: avoid;" class="a4-voucher-page pdf-container voucher-wrapper pdf-page-container voucher-root-container flex flex-col justify-between overflow-hidden p-2.5 bg-white text-slate-900 text-[9.5px] leading-tight">
      ...
      <div>
        <div class="flex items-center justify-between border-b-2 border-emerald-700 pb-1 mb-1">
          <div class="flex items-center space-x-1.5 max-w-[55%]">
            ${logoHtml}
            <div class="overflow-hidden">
              <h1 class="text-[10px] font-black text-emerald-950 tracking-tight uppercase truncate leading-tight">${agencySettings.agencyName || 'SAUDI PAK GROUP OF TRAVELS'}</h1>
              <p class="text-[8px] text-slate-700 font-bold truncate">${agencySettings.tagline || 'Hajj & Umrah Pilgrimage Services'}</p>
              <p class="text-[7.5px] text-emerald-800 font-extrabold truncate">${agencySettings.licenseNo ? 'Lic: ' + agencySettings.licenseNo : ''}</p>
            </div>
          </div>

          <div class="text-right text-[8.5px] space-y-0.1 text-slate-800 font-semibold max-w-[45%]">
            <p class="font-extrabold text-slate-900 truncate"><i class="fa-solid fa-phone text-emerald-700 mr-0.5"></i>${agencySettings.phone1 || ''}</p>
            <p class="truncate"><i class="fa-solid fa-envelope text-emerald-700 mr-0.5"></i>${agencySettings.email || ''}</p>
            <p class="truncate"><i class="fa-solid fa-globe text-emerald-700 mr-0.5"></i>${agencySettings.website || ''}</p>
            <p class="text-[7.5px] text-slate-700 truncate"><i class="fa-solid fa-location-dot text-emerald-700 mr-0.5"></i>${agencySettings.address || ''}</p>
          </div>
        </div>
        ...
        <div class="bg-emerald-800 text-white rounded px-2 py-0.5 flex items-center justify-between shadow-sm mb-1">
          <div class="flex items-center space-x-1">
            ...
            <span class="font-black text-[9px] tracking-wider uppercase">OFFICIAL TRAVEL & UMRAH VOUCHER</span>
          </div>
        ...
        <!-- SUMMARY INFO GRID -->
        <div class="grid grid-cols-4 gap-1 bg-emerald-50/80 border border-emerald-200 rounded p-1 mb-1 text-[9px]">
          <div>
          ...
          <div>
            ...
            <span class="px-1 py-0.1 bg-emerald-700 text-white rounded font-black text-[9px] inline-block">${data.totalPax} Person(s)</span>
          </div>
        </div>

        <!-- 1. PASSENGER BASIC DETAILS TABLE -->
        <div class="mb-1">
          <div class="bg-slate-800 text-white text-[9px] font-extrabold px-1.5 py-0.2 rounded-t flex items-center justify-between">
          ...
          <table class="w-full text-left border-collapse border border-slate-300 text-[9px] table-fixed">
            <thead>
            ...
            <tbody>
              ${passBasicRowsHtml || '<tr><td colspan="6" class="text-center py-0.5 font-bold text-slate-500">No passenger basic details listed</td></tr>'}
            </tbody>
          </table>
        </div>
        ...
        <div class="mb-1">
          <div class="bg-emerald-950 text-white text-[9px] font-extrabold px-1.5 py-0.2 rounded-t flex items-center justify-between">
          ...
          <table class="w-full text-left border-collapse border border-slate-300 text-[9px] table-fixed">
            ...
            <tbody>
              ${passVisaRowsHtml || '<tr><td colspan="6" class="text-center py-0.5 font-bold text-slate-500">No visa / MOFA details listed</td></tr>'}
            </tbody>
          </table>
        </div>` : ''}
        ...
        <div class="mb-1">
          <div class="bg-emerald-900 text-white text-[9px] font-extrabold px-1.5 py-0.2 rounded-t flex items-center justify-between">
          ...
          <table class="w-full text-left border-collapse border border-slate-300 text-[9px] table-fixed">
            ...
            <tbody>
              ${hotelRowsHtml || '<tr><td colspan="7" class="text-center py-0.5 font-bold text-slate-500">No accommodation details listed</td></tr>'}
            </tbody>
          </table>
        </div>
        ...
        <div class="border border-slate-300 rounded p-1 bg-slate-50 mb-1 text-[9px] space-y-0.2">
          <div class="font-black text-emerald-900 text-[9.5px] border-b border-slate-200 pb-0.2 flex items-center">
        ...
        <div class="flex items-center justify-between border-2 border-emerald-300 bg-emerald-50/90 rounded p-1 mb-1 text-[9px]">
          <div class="space-y-0.2 text-slate-900">
          ...
          <!-- 60px QR Code Badge for Compact Fit -->
      ...
      <div class="border-t-2 border-emerald-700 pt-0.5 text-[8.5px] text-slate-900 font-semibold space-y-0.2">
        <div class="grid grid-cols-2 gap-1">
          <!-- Urdu Rules -->
          <div dir="rtl" class="text-right font-arabic bg-slate-50 p-1 rounded border border-slate-200">
            <span class="font-black text-emerald-950 block text-[9px] mb-0.1">ضروری ہدایات و شرائط:</span>
            <ul class="list-disc pr-2 space-y-0.1 text-[8px] font-bold">
          ...
          <div class="bg-slate-50 p-1 rounded border border-slate-200">
            <span class="font-black text-emerald-950 block text-[9px] mb-0.1">Terms & Conditions:</span>
            <ul class="list-disc pl-2 space-y-0.1 text-[8px] font-bold">
