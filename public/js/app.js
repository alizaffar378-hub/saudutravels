...
async function downloadPreviewPDF(e) {
  ...
  try {
    let element = document.getElementById('voucher-preview-container') || document.getElementById('a4VoucherTemplate');
    if (!element || !element.innerHTML.trim() || element.children.length === 0) {
      const renderedHtml = await renderA4VoucherHTML(formData, currentAgencySettings);
      if (element) element.innerHTML = renderedHtml;
    }
...
// --- GENERATE AND SAVE VOUCHER RECORD ---
async function generateAndSaveVoucher(e) {
  ...
  try {
    ...
    if (result.success) {
      showToast(`Voucher ${formData.id} saved to record!`, 'success');
    }
  } catch (err) {
    console.warn("Backend save failed, saving to LocalStorage fallback: ", err);
    saveVoucherToLocalStorage(formData);
    showToast(`Voucher ${formData.id} saved to local storage!`, 'success');
  }
