...
function renderSavedVouchersTable(vouchers) {
  ...
  tbody.innerHTML = vouchers.map(v => {
    ...
    if (isAuthorizedToApprove && status === 'NOT APPROVED') {
      approveButton = `
        <button type="button" onclick="approveVoucher('${v.id}')" title="Approve Voucher" class="px-2.5 py-1.5 bg-teal-600 hover:bg-teal-700 text-white border border-teal-700 rounded text-xs font-bold shadow-sm min-h-[34px]">
      ...
      <tr class="hover:bg-emerald-50/40 transition-colors">
        ...
        <td class="py-2 px-3">
          <div class="flex items-center space-x-1.5">
            ${approveButton}
            <button type="button" onclick="previewSavedVoucher('${v.id}')" title="Preview PDF" class="px-2.5 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-300 rounded text-xs font-semibold min-h-[34px]">
              <i class="fa-solid fa-eye"></i> View
            </button>
            <button type="button" onclick="loadVoucherToForm('${v.id}')" title="Edit / Load to Form" class="px-2.5 py-1.5 bg-amber-50 text-amber-700 hover:bg-amber-100 border border-amber-300 rounded text-xs font-semibold min-h-[34px]">
              <i class="fa-solid fa-pen-to-square"></i> Edit
            </button>
            <button type="button" onclick="deleteSavedVoucher('${v.id}')" title="Delete" class="px-2.5 py-1.5 bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 rounded text-xs font-semibold min-h-[34px]">
              <i class="fa-solid fa-trash-can"></i>
            </button>
          </div>
        </td>
...
function renderDrawerVouchers(vouchers) {
  ...
  listContainer.innerHTML = vouchers.map(v => {
    ...
    if (isAuthorizedToApprove && status === 'NOT APPROVED') {
      approveButton = `
        <button type="button" onclick="approveVoucherFromDrawer('${v.id}')" title="Approve Voucher" class="px-2.5 py-1.5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded flex items-center space-x-1 min-h-[32px]">
      ...
      <div class="bg-white border border-slate-200 rounded-lg p-3 shadow-sm hover:border-emerald-300 transition-colors space-y-2">
        ...
        <div class="flex items-center justify-between border-t border-slate-100 pt-2 text-[10px]">
          ...
          <div class="flex space-x-1">
            ${approveButton}
            <button type="button" onclick="loadVoucherToFormFromDrawer('${v.id}')" title="Load into Form" class="px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-700 font-bold rounded flex items-center space-x-1 min-h-[32px]">
              <i class="fa-solid fa-pen-to-square"></i>
              <span>Edit</span>
            </button>
            <button type="button" onclick="reDownloadVoucherPDF('${v.id}')" title="Re-Download PDF" class="px-2.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 font-bold rounded flex items-center space-x-1 min-h-[32px]">
              <i class="fa-solid fa-download"></i>
              <span>PDF</span>
            </button>
            <button type="button" onclick="deleteVoucherFromDrawer('${v.id}')" title="Delete" class="px-2.5 py-1.5 bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 font-bold rounded flex items-center space-x-1 min-h-[32px]">
              <i class="fa-solid fa-trash-can"></i>
            </button>
          </div>
        </div>
    ...
    return `
      <tr class="hover:bg-slate-50 transition-colors">
        ...
        <td class="py-3 px-3 text-center">
          <button type="button" onclick="handleDeleteUser('${u.id}')" class="text-red-500 hover:text-red-700 p-2.5 font-bold inline-flex items-center justify-center min-h-[40px] min-w-[40px] rounded-lg hover:bg-red-50 transition-colors">
